...
from .lidar_encoder import build_lidar_encoder
...
class LiViP3D(MVXTwoStageDetector):
    def __init__(self,
                 embed_dims=256,
                 num_query=300,
                 num_classes=7,
        ...
        if lidar_encoder is not None:
            self.lidar_encoder = build_lidar_encoder(lidar_encoder)
        else:
        ...
        if self.do_pred:
            if self.use_map:
                ...
                from .predictor_no_map import CustomGraph
                self.predictor = CustomGraph(**predictor)
    ...
    def velo_update(self, ref_pts, velocity, l2g_r1, l2g_t1, l2g_r2, l2g_t2,
        ...
        ref_pts = (ref_pts - l2g_t2) @ torch.linalg.inv(l2g_r2.cpu()).T.type(torch.float).to(device=time_delta.device)
    ...
    @auto_fp16(apply_to=('img', 'radar', 'points'), out_fp32=True)
    def extract_feat(self, points, img, radar=None, img_metas=None):
        """Extract features from images and points."""
        device = next(self.parameters()).device
        if img is not None:
            img = img.to(device)
        if points is not None and self.lidar_encoder is not None:
        ...
        if radar is not None:
            radar_feats = None  # self.radar_encoder(radar)
        else:
            radar_feats = None
        if points is not None and self.lidar_encoder is not None:
            pts_feats = self.lidar_encoder(points)
        else:
            pts_feats = None
        ...
        if self.fix_feats:
            ...
            img_feats = self.extract_img_feat(img, img_metas)
        return (img_feats, radar_feats, pts_feats)
    ...
    def forward_train(self,
        ...
        if True:
            ...
            for i in range(num_frame):
                points_single = torch.stack([p_[i] for p_ in points], dim=0)
